...
/* return the bulge density for the given coordinate
 */
double bulge_get_density(double *pos)
{
  double r = sqrt(pos[0] * pos[0] + pos[1] * pos[1] + All.BulgeStretch * All.BulgeStretch * pos[2] * pos[2]);

  return All.BulgeStretch * All.Bulge_Mass / (2 * M_PI) * All.Bulge_A / (r + 1.0e-6 * All.Bulge_A) / pow(r + All.Bulge_A, 3);
}
