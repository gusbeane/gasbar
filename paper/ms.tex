...
been subtracted. The solid line indicates the direction of the bar while the
...
A mock stellar light image using filter profiles from the \textit{Hubble Space Telescope} is shown in
...
slowing it down\cite{1984MNRAS.209..729T, 1985MNRAS.213..451W,
...
reinforce the wake, arresting the process by which the halo can slow the bar
...
\item[Data Availability]  Simulation output at a selection of timesteps as
well as the initial conditions will be made publicly available. Simulation
output at higher timestep frequency will be available upon request.
...
repository (associated file \texttt{gasbar.tar.gz}).

\item[Supplementary Information] Supplementary information is available for
this manuscript.
...
the \texttt{MakeNewDisk} code\cite{2005MNRAS.361..776S}. The GALAKOS model has three
...
represent single stellar populations with a Chabrier initial mass function\cite{2003PASP..115..763C}. For each star particle, the deposition
