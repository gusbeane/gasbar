...
\caption{Surface density projections in an \Nbody{} only simulation (upper
panels) and a simulation which includes the SMUGGLE model (bottom panels). We
can see that in the \Nbody{} run, the bar grows in length and strength. In the
...
strength over the course of the simulation. The \Nbody{} model is identical to
...
\caption{Various bar properties from the \Nbody{} and SMUGGLE runs. The
\textit{upper panel} show the evolution of the pattern speed. As expected, the
bar in the \Nbody{} run slows down due to interactions between the bar and the
...
the evolution of the bar length. In the \Nbody{} case, the bar lengthens. This
...
show the torque exerted by the halo in both the \Nbody{} and SMUGGLE cases. The
dashed line shows the torque exerted by the gas phase in the SMUGGLE run
(there is no gas in the \Nbody{} run). Details on how these properties
...
\caption{The wake excited in the dark matter halo in the \Nbody{} case
...
angle difference in the SMUGGLE case is smaller than in the \Nbody{} case by
about a factor of two, reflecting how the dark matter halo in the SMUGGLE case
is unable to exert as negative a torque on the bar as in the \Nbody{}
case.}\label{fig:wake}
\end{figure*}
...
\caption{The circular velocity curve for the \Nbody{} run (blue) and the SMUGGLE
run (orange) compared to observational estimates for the Milky
...
Way.\cite{2022ApJ...929L..18E} The discrepancy between $1$ and
$4\,\textrm{kpc}$ is likely due to the fact that the distances to molecular
cloud which underlines this work used a simple kinematic distance based on an
axisymmetric model of the Milky Way,\cite{2017ApJ...834...57M} which is likely
not accurate in the bar region where gas has large non-circular velocities.
...
\caption{The time evolution of the molecular gas surface density (\textit{upper}) and
...
times during the simulation while the black dashed lines indicate models for
the Milky Way which allow the CO-to-H$_2$ conversion factor $X_{\textrm{CO}}$
to vary with metallicity.\cite{2022ApJ...929L..18E} We see that the molecular
gas and SFR surface densities are within an order of magnitude of the Milky
Way's typical values at all times.  We see a sharp decrease in the gas and SFR
surface densities along the extent of the bar from $\sim1$ to
$\sim4\,\textrm{kpc}$, related to the gas inflow in this region. {\color{red}
Waiting on atomic gas data from Juergen Kerp.}}
\label{fig:surf}
\end{figure}
...
\caption{The time evolution of the bar strength, measured as the maximum of the second
Fourier component divided by the zeroth Fourier component (a formula is given
in the Methods section). We see that in the \Nbody{} case (blue) the bar
...
projection through the stellar component of the \Nbody{} simulation (disk and
