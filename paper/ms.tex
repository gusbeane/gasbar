...
\begin{figure*}
    ...
    \SMUGGLE{} model for the interstellar medium. Each panel is
    $20\,\textrm{kpc}$ to a side. Columns show different points in time,
    separated by $1\,\textrm{Gyr}$. We can see that in the \Nbody{} run, the bar
    grows in length and strength. In the \SMUGGLE{} run, the bar remains at
    approximately the same length and strength over the course of the
    simulation. The \Nbody{} model is identical to the GALAKOS model {\bf (with
    different particle numbers and softening lengths)}, discussed in the text.
    Details on the production of these synthetic images is given in
    Appendix~\ref{app:hst}.}\label{fig:overview}
\end{figure*}
...
2019MNRAS.489.4233M}.} Snapshots were saved at equal intervals of $0.005$ in the
time units of the simulation, $\textrm{kpc}/(\textrm{km}/\textrm{s})$.
...
plot the circular velocity curve {\bf at $t=1\,\textrm{Gyr}$} in Fig.~\ref{fig:vcirc} compared to
...
{\bf We measured the initial scale height of the gas disk in a bin extending from
$R=8\,\textrm{kpc}$ to $R=8.5\,\textrm{kpc}$. The vertical profile is well-fit
by a Gaussian with a scale height of $97\,\textrm{pc}$. This is somewhat lower
than the observed value in the HI disk of $\sim200\,\textrm{kpc}$
\citep{1995ApJ...448..138M}.}
