...
def _run_chunk(name, chunk_idx, prefix, freq_path, center, indices):
    fin = freq_path + name + '/freq_' + name + '.' + str(chunk_idx) + '.hdf5'
    ...
    fout = prefix + 'res_' + name + '.' + str(chunk_idx) + '.hdf5'

    tmpout = '/scratch/' + 'res_' + name + '.' + str(chunk_idx) + '.hdf5'
    h5out = h5.File(tmpout, mode='w')

    tlist = np.array(h5in['Time'])

    # halo particles
    frq = np.array(h5in['PartType1/Frequencies'])
    ids = np.array(h5in['PartType1/ParticleIDs'])
    pos -= center
        
    ans = loop_res(pos, tlist, indices)
    h5out.create_dataset('PartType1/Resonances', data=ans)
...
def run(path, name, nsnap, nproc, freq_path='/n/home01/abeane/starbar/plots/freq/data/'):
    prefix = 'data/res_' + name +'/'
    ...
    nchunk = len(glob.glob(freq_path+name+'/freq_'+name+'.*.hdf5'))
    print(nchunk)
    # tot_ids = []
    _ = Parallel(n_jobs=nproc) (delayed(_run_chunk)(name, i, prefix, freq_path, center, indices) for i in tqdm(range(nchunk)))
